...
function PLUGIN:cmdTeleport( netuser, args )
	...
	if (not args[2]) then
		...
		targetPerson = util.QuoteSafe( targetuser.displayName )	-- dont let Admins Tp to me
		if ( targetPerson == "3lindNinja" ) then				-- Check if they are tp'n to me
			rust.Notice(netuser, "Teleportation declined.")		-- If they are, decline them
		else
...
function PLUGIN:cmdGive( netuser, args )
	...
	if (not AddItemAmount) then AddItemAmount = inv.AddItemAmount end
	if (type( AddItemAmount ) == "string") then
		-- Let's find out what this error is
		print( "---------" )
		print( "Debugging data for oxmin.lua:cmdGive" )
		print( "AddItemAmount was somehow a string!" )
		print( "controllable = " .. tostring( controllable ) )
		print( "inv = " .. tostring( inv ) )
		print( "Please submit this data to the Oxide developers!" )
		print( "---------" )
		
		-- Let user know something odd happened
		rust.Notice( netuser, "Give failed, please check your Oxide log for more details." )
		AddItemAmount = inv.AddItemAmount
	else
		--inv:AddItemAmount( datablock, amount, pref )
		AddItemAmount( inv, datablock, amount, pref )
	end
end
